...
const createWeb3Instance = (endpoint: string) => {
  const web3 = new Connection(endpoint, { commitment, confirmTransactionInitialTimeout: 60 })
  return web3
}
...
const web3Plugin: Plugin = async (ctx, inject) => {
  ...
  if (DEVNET_MODE) {
    ...
    let responseData
    try {
      responseData = await fetch('https://api.cropper.finance/rpc/').then((res) => res.json())
    } catch {
    } finally {
      endpoint = responseData.rpc
    }
  }
