...
export default Vue.extend({
  ...
  watch: {
    ...
    'token.initialized': {
      handler(newState: boolean) {
        this.toCoin = getTokenBySymbol(this.toForced)
        this.fromCoin = getTokenBySymbol(this.fromForced)
        const { from, to, ammId } = this.$route.query
        // @ts-ignore
        this.setCoinFromMint(ammId, from, to)
      },
  ...
  async mounted() {
    ...
    if (this.toCoin == null) {
      this.toCoin = Object.values(TOKENS).find((item) => item.symbol === this.toForced)
    }

    if (this.fromCoin == null) {
      this.fromCoin = Object.values(TOKENS).find((item) => item.symbol === this.fromForced)
    }
