<template>
  <div class="farm container">
    ...
    <div v-if="availableOptions.length > 0" class="note-unwrapped-sol">
      <div class="note-content">
        <img class="note-icon" src="@/assets/icons/status-warning.svg" />
        <label class="font-small weight-semi">
          You have {{availableOptions.length}} <span style="color: #23adb4">Option{{availableOptions.length > 1 ? 's' : ''}}</span> in your wallet.
          Click to exercise your option{{availableOptions.length > 1 ? 's' : ''}}.
        </label>
      </div>
      <Button v-if="availableOptions.length > 1" class="note-btn font-small weight-semi" @click="displayOptionSelector">Select</Button>
      <Button v-else class="note-btn font-small weight-semi" @click="exerciseFormOption(availableOptions[0])">Exercise</Button>
    </div>
    ...
    <OptionsSelector
      :show="optionSelectorDisplay && wallet.connected"
      :availableOptions="availableOptions"
      @onCancel="()=>{
        this.optionSelectorDisplay = false;
        this.getAvailableOptions();
      }"

      @onSelect="optionSelection"
    />

    <OptionsExercise
      :show="optionExerciseDisplay && wallet.connected"
      :option="selectedOption"
      @onCancel="()=>{
        this.optionExerciseDisplay = false;
        this.getAvailableOptions();
      }"
    />
    ...
    <div class="card">
      <div class="card-body">
        ...
        <div class="farm-content">
          <div class="farm-head fcsb-container noMobile">
            ...
            <div class="fcs-container">
              <div class="action-btn-group fcs-container">
                ...
                <a
                  ...
                  "
                >
                  <div class="create-plus-btn font-small weight-semi fcc-container">+ Create farm</div>
                </a>
          ...
          <div class="farm-option-bar fcsb-container">
            ...
            >
              ...
              <div
                v-if="showTabMenu"
                class="option-collapse-menu collapse-left"
                v-click-outside="
                  () => {
                  ...
                  <div class="state">
                    <div class="lp-icons">
                      <div class="lp-icons-group">
                        <div class="icons">
                          <NuxtLink style="cursor: initial;" :to="`/token/${farm.farmInfo.lp.coin.symbol}/`"><CoinIcon :mint-address="farm.farmInfo.lp.coin.mintAddress" /></NuxtLink>
                          <span class="font-medium weight-semi">{{ farm.farmInfo.lp.coin.symbol }} - </span>
                          <NuxtLink style="cursor: initial;" :to="`/token/${farm.farmInfo.lp.pc.symbol}/`"><CoinIcon :mint-address="farm.farmInfo.lp.pc.mintAddress" /></NuxtLink>
                  ...
                  <div class="state">
                    <div class="farm-labels">
                      ...
                      >
                        ...
                        <a v-if="farm.options" href="https://cropperfinance.medium.com/cropper-introduces-liquidity-mining-2-0-in-partnership-with-psyoption-d001dd5f86e7" target="_blank">
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      paying: false,
      TVL: 0,
      optionSelectorDisplay: false,
      optionExerciseDisplay: false,
  ...
  head: {
    title: 'Farms | Yield Farming - Cropper',
    meta: [
      {
        hid: 'og:site_name',
      ...
      },
      {
        hid: 'og:url',
        property: 'og:url',
        content: 'https://cropper.finance/farms/'
      },
      {
        hid: 'og:title',
        property: 'og:title',
        content: 'Farms | Yield Farming - Cropper'
      },
      {
        itemprop: 'name',
        content: 'Farms | Yield Farming - Cropper'
      ...
      },
      {
        hid: 'og:description',
        name: 'og:description',
        content: "Explore Cropper's Top APR% Labelized and Permissionless Farms. Farm now, harvest later!"
      },
      {
        hid: 'description',
  ...
  methods: {
    ...
    optionSelection(option: any){
        ...
        this.optionSelectorDisplay = false;
        this.optionExerciseDisplay = true;
    },

    async getAvailableOptions(){

      const program = usePsyAmericanProgram(this.$web3, this.$wallet);
      ...
      let apiDatas = await fetch('https://api.cropper.finance/options/').then((res) =>
              res.json())


      let projectList = apiDatas.allowed_options


      for(let i in options){
        if(
          projectList.includes(options[i].underlyingAssetMint.toString())
        ){

          if(this.$accessor.wallet.tokenAccounts[options[i].optionMint.toString()] && this.$accessor.wallet.tokenAccounts[options[i].optionMint.toString()].balance.wei.toNumber() > 0){
    ...
    displayOptionSelector(){
      this.optionSelectorDisplay = true;
    },

    exerciseFormOption(option: any){
      this.selectedOption = option;
      this.optionExerciseDisplay = true;
    },
    ...
    async updateFarms() {
      ...
      for (const [poolId, farmInfo] of Object.entries(this.farm.infos)) {
        ...
        if (reward && lp) {
          ...
          if(newFarmInfo.apr < 0){
            newFarmInfo.apr = newFarmInfo.apr_details.apy
          }
        ...
        if (userInfo && lp && FARM_VERSION === 1) {
          ...
          let { rewardDebt, depositBalance } = userInfo
          let currentTimestamp = this.currentTimestamp

          if(this.labelizedAmmsExtended[poolId] && this.labelizedAmmsExtended[poolId].options && this.labelizedAmmsExtended[poolId].options == true){
            rewardDebt = await YieldFarm.findUserInfoAccount(this.$web3, new PublicKey(FARM_PROGRAM_ID), new PublicKey(poolId), new PublicKey(this.wallet.address))

            rewardDebt = new TokenAmount(rewardDebt.rewardDebt, 0)
          ...
          let rewardPerShareCalc = rewardPerTimestamp
            .multipliedBy(duration)
            .multipliedBy(REWARD_MULTIPLER)
        ...
        if (
          ...
          if (lp) {
            ...
            if (this.labelizedAmms[newFarmInfo.poolId]) {
              ...
              if (this.labelizedAmmsExtended[newFarmInfo.poolId].options == true) {
                options0 = true
              }
    ...
    harvestAll_v2Farms() {
      let harvestableFarms = this.showFarms.filter(
